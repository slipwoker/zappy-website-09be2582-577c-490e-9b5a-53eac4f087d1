document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      if (type === 'tel' || field.name === 'phone' || field.name === 'tel') {
      ...
      if (field.minLength && field.minLength > 0 && value.length < field.minLength) {
        setFieldError(field, label + ' must be at least ' + field.minLength + ' characters.');
        return false;
      }

      if (field.maxLength && field.maxLength > 0 && value.length > field.maxLength) {
        setFieldError(field, label + ' must not exceed ' + field.maxLength + ' characters.');
        return false;
      }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "בד בבד משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '09be2582-577c-490e-9b5a-53eac4f087d1',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '09be2582-577c-490e-9b5a-53eac4f087d1';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '09be2582-577c-490e-9b5a-53eac4f087d1';
